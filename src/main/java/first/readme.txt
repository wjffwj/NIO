...
----------------------------------------------------------------------------------------------------------------------------------
Netty处理器概念：
  1.netty的处理器可以分为2类：入站处理器 和 出站处理器
  2.入栈处理器顶层是 channelInboundHandler 出战处理器顶层是 CHannelOutBoundHandler
  3.数据处理时各种编解码器都是处理器
  4.编解码器：网络传递时是以字节流形势存在 本来格式转为字节流叫编码    字节流转换为其本来格式叫解码
5.  编码：本质是出战处理器 因此编码器是一种 outBoundHandler
6.  解码：本质是入栈处理器 因此解码器是一种入栈处理器inBoundHandler
7   XXXEncoder   XXXDecoder   等命名
-------------------------------------------------------------------------------------------------------------------------
ByteToMessageDecoder
1.关于netty编解码器的重要结论
1.无论编码器还是解码器，其所接受的消息类型必须要与待处理的参数类型一致，否则该编码器或解码器并不会被执行
2.在解码器进行数据解码时一定要记得缓冲区数据是否足够，否则会有问题需要进行缓冲区大小判断

ReplayingDecoder:
是ByteToMessageDecoder的一种特殊变体，它可以在阻塞I / O范例中实现非阻塞解码器。
ReplayingDecoder和ByteToMessageDecoder之间的最大区别是，
ReplayingDecoder允许您像已收到所有必需的字节一样实现decode（）和decodeLast（）方法，而不是检查所需字节的可用性。

ReplayingDecoder传递了一个专门的ByteBuf实现，当缓冲区中的数据不足时，该实现将引发某种类型的Error。
 在上面的IntegerHeaderFrameDecoder中，您仅假设调用buf.readInt（）时缓冲区中将有4个或更多字节。 如果缓冲区中确实有4个字节，它将返回您期望的整数头。
  否则，将引发Error并将控件返回给ReplayingDecoder。 如果ReplayingDecoder捕获到错误，则它将把缓冲区的readerIndex倒回到“初始”位置（即缓冲区的开始），
  并在缓冲区中接收到更多数据时再次调用encode（..）方法。
请注意，ReplayingDecoder总是抛出相同的缓存Error实例，以避免创建新Error并为每次抛出填充其堆栈跟踪的开销。

LengthFieldBasedFrameDecoder
解码器按消息中的length字段的值动态拆分接收到的ByteBuf。 在解码二进制消息时，此消息特别有用，该二进制消息具有一个表示邮件正文或整个邮件长度的整数头字段。
LengthFieldBasedFrameDecoder具有许多配置参数，因此它可以解码带有长度字段的任何消息，这通常在专有的客户端-服务器协议中很常见。 以下是一些示例，可以使您基本了解哪个选项可以执行什么操作。
